...
const JobDetail = () => {
  ...
  const [setSimilarJobs, setSetSimilarJobs] = useState([]);
  // const [numOfApp, setNumOfApp] = useState("No applicants");
  ...
  const fetchJobDetails = async () => {
    ...
    try {
      ...
      setJob(res?.data);
      setSetSimilarJobs(res?.similarJobs);
      setIsFetching(false);
    } catch (error) {
      console.log(error);
      setIsFetching(false);
      ...
      <div className="w-full flex flex-col md:flex-row gap-10">
          ...
          <div className="w-full h-fit md:w-2/3 2xl:2/4 bg-white px-5 py-10 md:px-10 shadow-md">
            ...
            <div className="w-full">
              {user?._id === job?.company?._id ? (
                <CustomButton
                  title="Delete"
                  containerStyles={`w-full flex items-center justify-center text-white py-3 px-5 outline-none rounded-full text-base bg-black`}
        ...
        <div className="w-full md:w-1/3 2xl:w-2/4 p-5 mt-20 md:mt-0">
          ...
          <div className="w-full flex flex-wrap gap-4">
            {setSimilarJobs?.slice(0, 6).map((job, index) => {
              const data = {
